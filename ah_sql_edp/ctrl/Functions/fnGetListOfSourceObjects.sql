...
/*
SELECT * FROM [ctrl].[fnGetListOfSourceObjects]() 
...
CREATE FUNCTION [ctrl].[fnGetListOfSourceObjects]()
RETURNS TABLE 
AS
RETURN 
(
	WITH SourceObjects AS (
		SELECT DISTINCT 
			...
			AdlsContainerName, LandingPath, RawPath, StagePath, CuratedPath, 
		...
		FROM (	
			...
			FROM (				
					...
					WHERE S.[TargetFolderName] IN ('Cerner PIC','Cerner RCI','EPIC OHSU') 
